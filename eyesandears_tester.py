...
def ensure_window_resize_move_style(hwnd):
    if os.name != "nt":
        return False
    try:
        normalized_hwnd = int(hwnd or 0)
        ...
        style = int(_user32.GetWindowLongW(wintypes.HWND(normalized_hwnd), GWL_STYLE) or 0)
        desired = style | WS_THICKFRAME | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_SYSMENU
        if desired != style:
            _user32.SetWindowLongW(wintypes.HWND(normalized_hwnd), GWL_STYLE, desired)
        ...
        logger.debug("Could not enforce resize/move style for frameless window.", exc_info=True)
        return False
...
class AuthShellBridge:
    ...
    def _get_hwnd(self):
        cached = get_top_level_window_handle(self._hwnd_cache)
        if cached:
            self._hwnd_cache = int(cached)
            return int(cached)
        ...
        if not hwnd:
            hwnd = resolve_webview_window_hwnd(
                self._window,
                title=getattr(self._window, "title", ""),
                timeout_seconds=0.35,
            )
        hwnd = get_top_level_window_handle(hwnd)
        if hwnd:
    ...
    def prime_window_handle(self, timeout_seconds=2.0):
        ...
        if hwnd:
            self._hwnd_cache = int(hwnd)
            ensure_window_resize_move_style(hwnd)
    ...
    def minimize(self):
        try:
            hwnd = self._get_hwnd()
            if hwnd:
                _user32.ShowWindow(wintypes.HWND(int(hwnd)), SW_MINIMIZE)
    ...
    def toggle_maximize(self):
        try:
            ...
            hwnd = self._get_hwnd()
            if hwnd:
                normalized_hwnd = wintypes.HWND(int(hwnd))
                if self._manual_maximized:
                    if self._normal_bounds:
                        x, y, width, height = self._normal_bounds
                        position_native_window(int(hwnd), int(x), int(y), int(width), int(height), on_top=False)
                    else:
                        _user32.ShowWindow(normalized_hwnd, SW_RESTORE)
                    self._manual_maximized = False
                    return False

                if not self._normal_bounds:
                    rect = wintypes.RECT()
                    if _user32.GetWindowRect(normalized_hwnd, ctypes.byref(rect)):
                        self._normal_bounds = (
                            int(rect.left),
                            int(rect.top),
                            int(max(320, rect.right - rect.left)),
                            int(max(240, rect.bottom - rect.top)),
                        )
                ...
                try:
                    ...
                    monitor = ctypes.windll.user32.MonitorFromWindow(normalized_hwnd, 2)
                    monitor_info = MONITORINFO()
                    monitor_info.cbSize = ctypes.sizeof(MONITORINFO)
                ...
                position_native_window(int(hwnd), int(left), int(top), target_width, target_height, on_top=False)
    ...
    def start_move(self):
        ...
        try:
            ...
            if self._manual_maximized and self._normal_bounds:
                ...
                position_native_window(int(hwnd), int(x), int(y), int(width), int(height), on_top=False)
                self._manual_maximized = False
            _user32.SetForegroundWindow(normalized_hwnd)
            _user32.ReleaseCapture()
            _user32.SendMessageW(normalized_hwnd, WM_NCLBUTTONDOWN, HTCAPTION, 0)
        except Exception:
    ...
    def start_resize(self, edge):
        ...
        try:
            ...
            _user32.SetForegroundWindow(normalized_hwnd)
            _user32.ReleaseCapture()
            _user32.SendMessageW(normalized_hwnd, WM_NCLBUTTONDOWN, int(hit_test), 0)
        except Exception:
...
<body class="theme-dark">
  <div class="window-shell">
        <div class="window-drag-strip pywebview-drag-region" id="windowDragStrip"></div>
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      document.getElementById('windowDragZone').addEventListener('mousedown', async (event) => {
        ...
        await startWindowMove();
            ...
            document.getElementById('windowDragStrip').addEventListener('mousedown', async (event) => {
                if (event.button !== 0) return;
                await startWindowMove();
            });
...
def run_auth_shell_webview(initial_state):
    ...
    try:
        ...
        def _prepare(window_obj, bridge_obj):
            try:
                window_obj.show()
                bridge_obj.prime_window_handle(timeout_seconds=2.2)
            except Exception:
...
if __name__ == "__main__":
    ...
    try:
        startup_progress_open()
        startup_progress_wait(0.06)
        main()
